...
_HEXACLUS_CMAP = LinearSegmentedColormap.from_list(
    'hexaclus', [ '#093FB4','#FFFCFB', '#ED3500']
)
...
def plot_uncertainty_map_with_hatching(
    ...
    _pred_label = f'Mean Predicted Value (price/{unit})' if has_scaler else 'Mean Predicted Value'
    _std_label  = f'Std Dev (price/{unit})' if has_scaler else 'Std Dev'
    ...
    if len(high_uncert_gdf) > 0:
        high_uncert_gdf.plot(ax=ax, facecolor='none', edgecolor='black',
                             linewidth=2.5, hatch='////', alpha=0.85,
...
def _compute_error(polygon_data_list: List[Dict], label_scaler=None, label_cols=None,
    ...
    return mean_absolute_error(all_y, all_pred)
...
def compute_feature_importance(
    ...
    if label_scaler is not None and label_cols is not None:
        _scale_unit = f'(price/{unit})²' if metric == 'mse' else f'(price/{unit})'
    else:
        _scale_unit = '(normalized)'
    _mname = metric.upper()

    # baseline score
    baseline_score = _compute_error(polygon_data, label_scaler, label_cols, metric)
    print(f"Baseline {_mname} {_scale_unit}: {baseline_score:.4f}")
    ...
    for feat_idx, feat_name in enumerate(feature_names):
        ...
        for repeat in range(n_repeats):
            ...
            permuted_score = _compute_error(permuted_data, label_scaler, label_cols, metric)
...
def plot_feature_importance(
    ...
    ax1.set_yticklabels(top_features['feature'], fontsize=25)
    ax1.tick_params(axis='x', labelsize=25)
    ax1.xaxis.get_offset_text().set_fontsize(25)
    ax1.set_xlabel('Importance (Mean ± Std)', fontsize=25)
    ax1.set_title(f'Top {top_n} Individual Features' + title, fontsize=25 + 5, fontweight='bold')
    ax1.invert_yaxis()
    ...
    if grouped_df is not None and ax2 is not None:
        ...
        ax2.set_yticklabels([f"{row['group']} ({row['n_features']})"
                             for _, row in grouped_df.iterrows()], fontsize=25)
        ax2.tick_params(axis='x', labelsize=25)
        ax2.xaxis.get_offset_text().set_fontsize(25)
        ax2.set_xlabel('Importance (Mean ± Std)', fontsize=25)
        ax2.set_title('Feature Group Importance' + title, fontsize=25 + 5, fontweight='bold')
        ax2.invert_yaxis()
...
def compute_regional_feature_importance(
    ...
    has_scaler = (label_scaler is not None) and (label_cols is not None)
    polygon_data, feature_names, _ = _prepare_polygon_data(clustering_model, test_instances)
    _err = (lambda a, b: float(np.mean(np.abs(a - b)))) if metric == 'mae' \
        else (lambda a, b: float(np.mean((a - b) ** 2)))
    ...
    for data in polygon_data:
        ...
        baseline_err = _err(y_eval, pred_eval)
        ...
        for group_name, group_features in feature_groups.items():
            ...
            for _ in range(n_repeats):
                ...
                permuted_err = _err(y_eval, pred_perm_eval)
                importance_scores.append(permuted_err - baseline_err)
...
def plot_regional_feature_importance(
    ...
    for poly_idx in poly_indices:
        ...
        if total > 0:
            ...
            percentages = [1.0 / n_groups] * n_groups  
        ...
        current_x = bar_x
        for i, (pct, bcolor) in enumerate(zip(percentages, colors)):
            ...
            if segment_width > 0:
                ...
                rect = Rectangle(
                    ...
                    facecolor=bcolor, edgecolor='black', linewidth=0.5,
    ...
    _cbar_label = f'Mean Predicted Value (price/{unit})' if has_scaler else 'Mean Predicted Value'
...
def plot_calibration_comparison(
    ...
    for idx, ((model_name, result), color) in enumerate(zip(results.items(), colors)):
        ...
        mk = marker_styles[idx % len(marker_styles)]

        # Plot calibration curve with unique line style + marker for B&W readability
        ax.plot(
    ...
    ax.plot([0, max_val], [0, mad_factor * max_val], 'k--', linewidth=2.5,
    ...
    guide_text = (
        "Closer to reference line = better calibrated\n"
        "Above line = underconfident, below = overconfident\n"
